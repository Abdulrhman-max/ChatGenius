...
def _get_or_create_conversation(admin_id, channel_type, external_id, sender_name="", phone="", email=""):
    ...
    row = conn.execute(
        "SELECT * FROM channel_conversations WHERE admin_id = ? AND channel_type = ? AND external_id = ?",
    ...
    if row:
        ...
        conn.execute(
            "UPDATE channel_conversations SET last_message_at = ?, unread_count = unread_count + 1 WHERE id = ?",
...
def send_reply(conversation_id, text, staff_name="Staff"):
    ...
    conn = db.get_db()
    conv = conn.execute("SELECT * FROM channel_conversations WHERE id = ?", (conversation_id,)).fetchone()
    conn.close()
...
def get_conversations(admin_id, channel_type=None, unread_only=False,
    ...
    conn = db.get_db()
    query = "SELECT * FROM channel_conversations WHERE admin_id = ?"
    params = [admin_id]
    ...
    if search:
        query += " AND (sender_name LIKE ? OR phone LIKE ? OR email LIKE ?)"
...
def get_conversation_messages(conversation_id, limit=50, offset=0):
    ...
    conn = db.get_db()
    rows = conn.execute(
        "SELECT * FROM channel_messages WHERE conversation_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?",
        (conversation_id, limit, offset)
    ).fetchall()

    # Mark as read
    conn.execute(
        "UPDATE channel_conversations SET unread_count = 0 WHERE id = ?",
        (conversation_id,)
    )
    conn.commit()
    conn.close()
...
def assign_conversation(conversation_id, staff_user_id):
    ...
    conn = db.get_db()
    conn.execute(
        "UPDATE channel_conversations SET assigned_to = ? WHERE id = ?",
...
def tag_conversation(conversation_id, tag):
    ...
    conn = db.get_db()
    conv = conn.execute("SELECT tags FROM channel_conversations WHERE id = ?", (conversation_id,)).fetchone()
    if conv:
        ...
        if tag not in tags:
            tags.append(tag)
        conn.execute(
            "UPDATE channel_conversations SET tags = ? WHERE id = ?",
...
def resolve_conversation(conversation_id):
    ...
    conn = db.get_db()
    conn.execute(
        "UPDATE channel_conversations SET status = 'resolved', resolved_at = ? WHERE id = ?",
...
def get_inbox_stats(admin_id):
    ...
    total = conn.execute(
        "SELECT COUNT(*) as c FROM channel_conversations WHERE admin_id = ?", (admin_id,)
    ).fetchone()["c"]

    unread = conn.execute(
        "SELECT COUNT(*) as c FROM channel_conversations WHERE admin_id = ? AND unread_count > 0",
        (admin_id,)
    ).fetchone()["c"]

    by_channel = conn.execute(
        "SELECT channel_type, COUNT(*) as c FROM channel_conversations WHERE admin_id = ? GROUP BY channel_type",
        (admin_id,)
    ).fetchall()
